...
public sealed class ShipEditorBatch : ICallbacks
{
    private static ShipEditorBatch active;
    ...
    public static string Run(string label = "r1", string filter = "ship")
    {
        ...
        batch.api.RetrieveTestList(TestMode.EditMode, root =>
        {
            ...
            var retry = filter.StartsWith("failed-") ? JObject.Parse(File.ReadAllText(Path.GetFullPath("../artifacts/walkable-ship/editor-" + filter.Substring(7) + ".json")))["results"]
            ...
            void Visit(ITestAdaptor test)
            {
                if (!test.IsSuite && test.FullName.StartsWith("DarkNights.Tests.") &&
                    ...
                    (retry != null ? retry.Contains(test.FullName) : filter == "all" || filter == "ship" && (test.FullName.Contains(".Ship") || test.FullName.Contains(".ExpeditionTests.")) || filter != "ship" && test.FullName.Contains(filter))) names.Add(test.FullName);
