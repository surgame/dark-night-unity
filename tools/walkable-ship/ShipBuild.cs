...
public static class ShipBuild
{
    public static string Run(string label = "")
    {
        string result = Path.GetFullPath("../artifacts/walkable-ship/build-result" + label + ".json");
        ...
        File.WriteAllText(Path.GetFullPath("../artifacts/walkable-ship/build-started.json"), DateTime.UtcNow.ToString("O"));
            var report = new JObject { ["startedUtc"] = DateTime.UtcNow.ToString("O"), ["backend"] = "Mono" };
            try
            {
                string output = Path.GetFullPath("../artifacts/walkable-ship/player-mono/DarkNights.exe");
