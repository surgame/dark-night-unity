...
public static class ExpeditionBuild
{
    public static string Run()
    {
        string result = Path.GetFullPath("../artifacts/expedition/build-result.json");
        ...
        File.WriteAllText(Path.GetFullPath("../artifacts/expedition/build-started.json"), DateTime.UtcNow.ToString("O"));
            var report = new JObject { ["startedUtc"] = DateTime.UtcNow.ToString("O"), ["backend"] = "Mono" };
            try
            {
                string output = Path.GetFullPath("../artifacts/expedition/player-mono/DarkNights.exe");
