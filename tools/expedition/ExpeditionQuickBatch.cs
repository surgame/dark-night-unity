...
public sealed class ExpeditionQuickBatch : ICallbacks
{
    private static ExpeditionQuickBatch active;
    ...
    private static string Output => Path.GetFullPath("../artifacts/expedition/editor-final.json");
    public static string Run()
    {
        if (active != null) throw new InvalidOperationException("Batch is already running");
        var batch = active = new ExpeditionQuickBatch();
        ...
        batch.api.RetrieveTestList(TestMode.EditMode, root =>
        {
            ...
            void Visit(ITestAdaptor test)
            {
                if (!test.IsSuite && test.FullName.StartsWith("DarkNights.Tests.") &&
                    !test.FullName.Contains("PlayTests")) names.Add(test.FullName);
                if (test.Children != null) foreach (var child in test.Children) Visit(child);
            }
            Visit(root); batch.scheduled = names.Count; batch.deadline = EditorApplication.timeSinceStartup + 120;
        ...
        return "Current Editor batch submitted; bounded to 120 seconds; " + Output;
    }
