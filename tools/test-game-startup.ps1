...
try {
    ...
    # 主角三命令加 AnyRuleD 地形命令；会话投影及七类业务状态，共 8 个 State。
    $checks['formal_object_content_loaded_once'] = [regex]::Matches($text, 'DARK_NIGHTS_FORMAL_CONTENT_READY definitions=28 commands=4 states=8 behaviours=29 identity=GuidFirst wire=GuidV2').Count -eq 1
