param([Parameter(Mandatory)][string]$PlayerPath, [int]$Port = 28840, [string]$Python = 'python')
...
try {
    ...
    & (Join-Path $PSScriptRoot 'test-game-terrain-loop.ps1') -PlayerPath $PlayerPath -Port $Port -ClientPort ($Port + 1)
} finally {
