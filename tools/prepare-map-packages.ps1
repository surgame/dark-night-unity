...
$destination = Join-Path $root '.deps/AnyRules'
...
if (Test-Path $destination) {
    ...
    $archive = Join-Path $root '.deps/anyrules-source.zip'
...
try {
    foreach ($name in @('IdleMapPublication','RealtimeMapRetry')) {
        ...
        if ($LASTEXITCODE -ne 0) {
            git apply --directory=.deps/AnyRules --check --ignore-space-change $patch
            if ($LASTEXITCODE) { throw 'AnyRules patch does not match locked source; preserve local changes.' }
            git apply --directory=.deps/AnyRules --ignore-space-change $patch
