...
$run = Join-Path $repo ('artifacts/contour/network-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + $(if ($Weak) {'-weak'} else {'-normal'}))
...
try {
    ...
    $backgroundHash = $h.terrain.backgroundHash
    ...
    Check 'late_initial_reference_matches' ($late.terrain.backgroundHash -eq $backgroundHash)
    if (!$SmokeOnly) {
        $hero = Actor $h 0
        ...
        $h = Wait-Report 'host' {param($r) (Actor $r 0).X -ge $deposit.X - 14} 'walk to starter ore'
        ...
        $h = Wait-Report 'host' {param($r) !(Actor $r 0).Walking} 'stop beside ore'
        ...
        do {
            Send 'host' @{operation='input-hold';actor=$hero.Id;horizontal=-1;jumpPressed=$true;jumpHeld=$true}
    ...
    Check 'reconnect_reference_matches' ($c.terrain.backgroundHash -eq $backgroundHash)
    ...
    Check 'automatic_save_exists' (Test-Path -LiteralPath (Join-Path $saves 'v9/slot-09.dnsave.json'))
    if (!$SmokeOnly) {
    ...
    $null = Wait-Report 'host' {param($r) !$r.storageBusy -and (Test-Path -LiteralPath (Join-Path $saves 'v9/slot-00.dnsave.json'))} 'disk save'
    ...
    Check 'restart_reference_matches' ($h.terrain.backgroundHash -eq $backgroundHash)
