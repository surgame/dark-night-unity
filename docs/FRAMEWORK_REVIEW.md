...
**处置：M2 必做。** 增加游戏专用 `CampCommandEndpoint`，从服务端连接取得身份，进入验证和串行队列。首版不把经济／战斗命令直接交给无身份上下文的全局 LocalInput handler。通用上下文增强可在后续回收至 YYGC，避免本轮扩大框架重写。
...
**处置：M2/M4。** 用会话级冻结快照和 Ready 门禁解决世界一致性；保留对象同步能力供其他合适对象使用。
...
当前 Tests 主要覆盖双轨池化、DI、UI 绑定和 YYArchive。未在已检查测试中找到 Host＋独立客户端、晚加入／重连、带身份业务命令、丢包恢复和 Player 构建证据。本轮未运行这些测试。
