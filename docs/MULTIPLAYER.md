...
状态：正式玩法联机设计，尚未实现；已有 Sample 的验证范围见上文。沿用 2–4 人合作、共享一个营地，按 Windows、房主主持、局域网／直连估算。本次新增可关闭的共享控制合同；公网房间、邀请、中继与平台身份不作为首版前提，见[移植方案](MIGRATION_PLAN.md)。

实现沿用 YYGC `10b8f0e` 在 Sample 中已验证的 Gateway/Sender/Processor 和 StatefulBehaviour/StateSynchronizer。先以一个会话状态承载有界、可靠的完整营地投影，验证正式实体集合的首次状态和在线替换；后文的分块、结构/运动拆流、增量暂存流程是在测量超限后启用的设计。权限、epoch、原子应用、Ready 与恢复合同从首个切片就必须成立。
...
| 居民控制 | 默认 `SharedCamp`，已 Ready 玩家可操作友军；可切 `HostOnly`，非房主只观看和查看信息；不分配私人单位 |
...
共享控制默认值和房主权限是设计建议，尚未实现。用户允许暂缓全员操作，也允许先开启后关闭；两种模式使用相同模拟与网络结构。相对单人的变化集中于权限、本地菜单、请求延迟与冲突反馈；不隐式改变战斗和经济规则。
...
1. 开房时写入模式，默认 `SharedCamp`；也可仅通过配置将首个切片设为 `HostOnly`。加入者完成 Ready 前先取得当前模式和策略版本。
...
6. 世界加载保留当前房间模式，不能让旧存档扩大来宾权限；加载增加 epoch 并重新 Ready。新建房间使用所选模式，网络待确认请求不写入存档。
...
sequenceDiagram
    ...
    C->>C: 验证并原子建立只读副本
    C->>H: SnapshotApplied(E,R) / Ready
    H-->>C: Ready 确认，按当前权限开放指令
