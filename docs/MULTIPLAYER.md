...
可破坏地图新增独立的[权威／同步／本地渲染规划与基础 API](TERRAIN_GENERATION.md)。地图采用 AnyRuleD AMP1 按兴趣区可靠同步当前块；破坏请求经 YYGC 可信命令链，客户端渲染不授予写权限。静止时由内容和权限版本跳过发布扫描；重试按实时时间推进。正式营地协议仍为 8、存档仍为 v3，采矿输入、人物碰撞、奖励和跨模块恢复尚未接入。
