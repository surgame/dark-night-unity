...
2026-09-16 当前实现采用协议 **8**／存档 **v3**，YYGC 锁定输入分支 `0c7cec0`。新增主角独占控制和有界输入流，复用 Gateway、可信连接、完整投影及同一权威 ActorState；暂停、策略切换、断线和加载使旧控制失效。实现与本批实际验证见[联合执行文档](HERO_INPUT_EXECUTION.md)。下方带旧日期与提交的统计是历史证据。
...
| 居民控制 | 默认 `SharedCamp`；每位 Ready 玩家可独占接管一名友军，其余保持自动控制。营地模式继续共享命令；`HostOnly` 会撤销来宾接管并拒绝其修改 |
