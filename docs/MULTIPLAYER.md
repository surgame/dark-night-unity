...
2026-09-16 当前实现采用协议 **8**／存档 **v3**，YYGC 锁定输入分支 `0c7cec0`。新增主角独占控制和有界输入流，复用 Gateway、可信连接、完整投影及同一权威 ActorState；Ready 携带默认主角偏好，服务端为每个有权限玩家分配不同的可用友军。暂停、策略切换、断线和加载使旧控制失效；加载、重连及 SharedCamp 恢复后按偏好重新分配。默认 UI 隐藏顶部主角工具栏与旧营地操作入口，输入快捷键和服务端权限合同继续生效。实现与本批实际验证见[联合执行文档](HERO_INPUT_EXECUTION.md)。下方带旧日期与提交的统计是历史证据。
...
| 居民控制 | 默认 `SharedCamp`；每位请求主角的 Ready 玩家由服务端直接分配一名不同的可用友军，其余保持自动控制。产品 UI 暂时隐藏顶部主角工具栏和营地命令，输入快捷键不受影响；旧 UI／后端仍受共享策略校验。`HostOnly` 会撤销来宾接管并拒绝其修改，恢复 SharedCamp 后按偏好补分配 |
...
6. 世界加载保留当前房间模式，不能让旧存档扩大来宾权限；加载增加 epoch 并重新 Ready，服务端只为当前策略允许且仍请求主角的连接重新分配。新建房间使用所选模式，网络待确认请求和分配偏好不写入存档。
...
4. 向活动玩家发送同一份新基线，设置 Ready 超时；每个成功 Ready 的连接按当前策略和请求偏好重新分配默认人物，超时者退出 Ready 集合，不能无限阻塞其余玩家。
