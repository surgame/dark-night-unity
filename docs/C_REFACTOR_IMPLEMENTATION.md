...
2026-09-12，执行基线 `e34f2a28`。[原执行方案](C_REFACTOR_PLAN.md)的 R1–R3 已实施，现交付用户验收架构。用户要求本轮尽快收尾，剩余问题统一留待下次；R4 不宣称完整验收通过。最终 Ready 修复后的 Mono 已构建并通过基础双进程回归，完整矩阵的版本边界及待办见下表。性能 A/C 对比留待后续；本批不推进 M5 既有画面修正，不构建 IL2CPP，不宣称双机器 LAN 或 M5 完成。
...
容量回归暴露既有 Ready 重试边界：每秒重试更换序号，可能持续丢弃较早请求的成功回执，出现服务端已 Ready 而客户端未 Ready。现同一 epoch 握手重试复用逻辑请求序号，成功后忽略迟到的拒绝；新 epoch 重置握手序号，连接和 epoch 过滤保持。新增 `SessionClientReadyTests` 四项回归；该修正改变运行输入，因此重新构建 Mono，Editor、真实 Play、启动及双进程通过。用户指定先收尾，容量与完整弱网复验留待下次。
...
本批原始记录位于 `Artifacts/c-refactor/`，最终摘要见 [实施证据](evidence/c-refactor-2026-09-12.json)。旧回归夹具与历史通过记录未重生成。
...
| Ready 修复前的 C 功能矩阵 | 并发 13/13、active-load 13/13、九组四进程弱网各 22/22、战斗 9/9、三夜 17/17；最终 Ready 修复后尚未完整重跑 |
| 画面基本回归 | 1280×800、1600×900 各 6/6，并人工查看日夜、菜单、暂停、帮助截图；M5 既有字体与布局问题未处理 |
| 合成容量上限 | 256 实体／1024 箭矢三客户端完整显示通过；发布断言失败与后续 Ready 超时保留，最终容量验收待下次 |
...
- [ ] 用最终 Ready 修复后的 Mono 重跑容量上限，保持 30 秒内发布增量大于 150 的原门槛。首次失败未保存断言操作数，不能确认根因；脚本现重试瞬时文件访问失败，并保存起止发布／tick 和测量窗口，先收集 metrics 再报告断言失败。
- [ ] 复验最终 Ready 修改的并发、加载、九组四进程弱网、战斗和三夜流程；此前 C 主体通过记录有效，但不能替代最终输入的完整验收。
