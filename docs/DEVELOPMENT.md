...
2026-09-16 已完成[默认主角入口收尾](HERO_INPUT_EXECUTION.md)：协议 8／存档 v3 和 YYGC `0c7cec0` 不变。Ready 命令携带本地默认主角偏好，服务端按稳定对象顺序为每个有权限玩家分配不同的可用友军；加载、重连及恢复 SharedCamp 后重新执行，HostOnly 仍撤销来宾控制。默认 UI 隐藏会遮挡画面的顶部主角工具栏和旧建造、训练、招募、修缮入口，主角快捷键保持可用；旧营地后端与工具栏只供 `--dn-camp-mode` 和回归使用。受影响 Editor／Play 按组 22/22，累计 175 个不同游戏用例按影响合并通过；架构守卫 316 文件／12 自测／0 错误。当前 Mono `artifacts/hero-input/player-mono-default-hero-r3` 构建成功，本机独立 Host＋客户端及两张实际 UI 捕获 37/37，默认截图已复核不再出现顶部框。输入 Sample 34 项及历史 350／155 项未因本次入口收尾重跑；前台性能、IL2CPP、双机器 LAN 与 M5 状态不变。
