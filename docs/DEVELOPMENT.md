...
当前停点：干净目录 Core 1355、Editor 55 均通过，新 Mono 构建先遇到并行编译内存不足，随后被本机 Unity 许可握手超时阻塞。新 UI／性能校准尚未完成 Player 验收；用户暂时没有第二台 Windows 电脑。逐项后续步骤、检查内容与是否仍需修改见 [M5 当前停点清单](M5_EXECUTION.md#当前停点与后续检查清单2026-09-12)，不能将已有旧 Player 的通过记录用于新源码签署。
...
当前按 [M5 连续路线](M5_EXECUTION.md#连续执行步骤)推进：A2 网络及 A3 必要操作已接入，继续补齐 M3 完整表现、M4 存档与恢复，并完成正式四进程／弱网矩阵。M1 存档产品收尾并入 M4；IL2CPP 和双机器分别取得对应前置条件后实测。
