...
2026-09-20 洞穴后续目标见[参考图差异与分阶段交付](CAVE_EXPLORATION_TARGETS.md)：A 固定视觉样板 → B 独立形状与真实角色 → C 随机空间构成 → D 会话接入。优先在同一小样中完成 A／B；当前均待实施。本轮仅完成评估及 AGENTS 生图适用性规范，未运行 Unity 或新增美术。
