...
计划更新：2026-09-12，配合[移植方案](MIGRATION_PLAN.md)。**M0 已完成；规则／存储基础、正式双进程网络、15 类原生外观、五页 UGUI 和鼠标操作已有实现及分批验证。M2 完整并发与网络矩阵、M3 完整表现、M4 恢复和 M5 交付尚未完成。** 下文统一使用 M0 表示环境与正式接入收口、M1 表示规则核心；历史记录中的“M0/M1 环境完成”不代表本表 M1 完成。
