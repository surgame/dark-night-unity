...
- `D:/Downloads/飞船/游戏接入验证_20260922/compile` 与 `static` 曾包含约 424 MiB 可重建 bin／obj；删除操作被自动审批以 `blocked by policy` 拦截后未重试。2026-09-22 曾按要求将清单中的 22 个目录、共 444,721,300 字节移动到 `D:/Downloads/飞船/临时待删除/20260922-walkable-expedition-ship/游戏接入验证_20260922`；当日晚间复核该 HoldingRoot 载荷已不在磁盘且回收站无记录（推定由用户永久删除）。验证目录剩余生成 csproj、日志及与仓库同源的 Program.cs 副本经核验无独立价值，已整目录移除；唯一的静态报告 `architecture.json` 与 `core-regression.json` 归档至 `docs/evidence/walkable-ship-2026-09-22/`。原始预览在 `artifacts/walkable-ship/cleanup-preview.json`，移动与复核记录见 `docs/evidence/walkable-ship-2026-09-22/retained-compile-outputs.json`。
- `C:/Users/Jobscn/.codex/worktrees/walkable-expedition-ship` 薄 worktree 已于 2026-09-22 按用户指示移除：其为分支可达提交 `70e4e7a` 的干净检出，无独立提交或未跟踪内容；其中首次 C# 试编的 CoreBuild bin／obj 共 425,004 字节先移入 `D:/Downloads/飞船/临时待删除/20260922-walkable-expedition-ship/codex-worktree-unity-projects/tools/CoreBuild/`（记录见同目录 `inventory.json`），随后经 `git worktree remove` 移除整个 worktree，源码与 `CoreBuild.csproj` 均保留在 git 分支内。
