...
右侧一次异步烘焙 **2560×1536 原生像素完整初始地图**，默认适配画布；中键拖动和滚轮缩放只变换已生成的完整纹理，不再请求局部画面，旧画面在新烘焙完整结束前保留并标示更新状态。参数停止变化约 0.4 秒后重新烘焙，连续变化取消旧任务；手动重烘焙仍可用。完整烘焙的 CPU／内存峰值和交互延迟尚待实测，不把画布 FPS 当作 Player 性能。此工具不运行 Play、不保存场景、不改地图格子、网络或存档。**这是无动态灯光的材质预览，底墙为简化颜色；角色、光照及最终合成仍须在 Play 核对。** 窗口不覆盖旧的场景静态预览图，也不自动导出 PNG。

2026-09-23 先前窗口布局曾为左侧可滚动资产 Inspector、右侧 504×312 裁剪画布；松开平移后按新取景位置重新烘焙。当前版本已改为上述全图缓存与自适应完整显示；滚轮仍围绕指针缩放（0.5–8 倍）。画布左上方显示编辑器画布刷新 FPS 和最近一次烘焙耗时；该 FPS **不是**游戏 Player／前台性能指标。原 `Map generator` 菜单和窗口标题标记为“旧版”，其生成／导出行为不变。
