...
右侧一次异步烘焙 **2560×1536 原生像素完整初始地图**，默认适配画布；中键拖动和滚轮缩放只变换已生成的完整纹理，不再请求局部画面，旧画面在新烘焙完整结束前保留并标示更新状态。参数或地图草稿停止变化约 0.4 秒后重新烘焙，连续变化取消旧任务；手动重烘焙仍可用。完整烘焙的 CPU／内存峰值和交互延迟尚待实测，不把画布 FPS 当作 Player 性能。此工具不运行 Play、不保存场景；地图格子仅在显式“应用地图”后写回初始资产，不改网络或存档。**这是无动态灯光的材质预览，底墙为简化颜色；角色、光照及最终合成仍须在 Play 核对。** 窗口不覆盖旧的场景静态预览图，也不自动导出 PNG。
...
2026-09-24 拆填刷新修正：运行场景的逻辑格与贴图使用 AnyRuleD 局部更新，但 Cave Wall Tuner 是独立的编辑态 RGBA 烘焙窗口。此前每次有效笔触在 0.4 秒静默后重算 2560×1536 前景、三层初始背景及整图合成，再上传整张纹理，因而没有享受 AnyRuleD 的局部刷新。窗口现在保存完整画面、冻结背景和前景轮廓快照；拆填在约 0.12 秒静默后只重算受影响的 256px 前景页，移除岩壁后直接露出缓存背景。开启全局 Modifier 时仍须重算一次完整轮廓以维持跨页锚点优先级，之后用轮廓差异定位页；样式／背景参数与源地图变动仍完整烘焙并保留 0.4 秒合并等待。连续笔触保留取消旧任务、只发布最新结果。合成后的 Texture2D 仍整张上传；编辑器前台交互耗时待 Unity 实测。独立 C# 9 合成回归中，圆簇例子的局部／完整计算约 522／904ms，空 Modifier 约 20／806ms，更新结果与完整烘焙逐字节一致；这些是合成输入的单机计算时间，不代表 Unity 窗口或 Player 性能。本次新增的 EditMode 用例已通过定向编译，尚未在当前主 Editor 内执行。
