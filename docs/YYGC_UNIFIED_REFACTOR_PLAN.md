...
本次交付为执行文档与迁移分支。已完成源码核对和分支创建，**U1–U6 尚未实施，也未运行本方案的 Unity／Player 验收**。本计划接续最后一轮架构评估，取代 [C 方案](C_REFACTOR_PLAN.md)作为后续重构方向；C 方案和[场景入口修复](SCENE_DEFINITIONS.md)仍是现有实现及历史证据。
...
下表名称为拟议实现名，尚未创建。具体拆分可以在 U2 根据代码体量调整；状态所有权和删除旧模型的门槛不变。
...
| 阶段 | 可审查交付 | 预计人日 | 当前状态 |
|---|---|---:|---|
| U0 分支与基线 | 决策、输入记录、比较方法、阶段验收门槛 | 0.5–1 | 分支／源码核对／本文已完成；运行基线与输入归档待执行 |
| U1 YYGC 前置能力 | 状态权限、显式会话装配、同步创建、严格配置检查 | 1.5–3 | 待执行 |
| U2 工人／采集／建造切片 | 一套对象模型的可操作、可保存、可联机闭环 | 2–3 | 待执行 |
| U3 全部玩法能力迁移 | 15 类正式定义、完整权威状态和三夜流程 | 3–5 | 待执行 |
| U4 场景／网络／存档产品接线 | 正式场景原实例、协议 6、新档 v2、UI 和会话恢复 | 2–4 | 待执行 |
| U5 旧模型退出与工具收口 | 删除旧运行实体和兼容链，更新守卫与生成入口 | 1–2 | 待执行 |
| U6 完整验收与交付 | 同一最终 Mono 产物的完整矩阵、性能记录、提交 | 2–4 | 待执行 |
...
以下路径从 `Game/Assets/DarkNights/Scripts` 起算。保留表示保留相应职责，不承诺不修改源码；替代项均待实施。
...
| `Runtime/Network/WorldSessionBehaviour.cs`、`SessionStatusState.cs`、`SessionProjector.cs` | 保留会话投影传输职责，重定新状态来源和载荷 | 一份冻结投影，生成注册与协议 6 一致 |
...
表内是必须取得的新证据，当前均未执行。测试数随有效覆盖变化，不能照抄旧的 1361／95 等数量作为目标或已通过结果。
...
| 新格式边界 | v2 往返、坏字段／关系、超限、取消、文件锁、旧 Godot／Unity v1 和协议 5 拒绝 | 新格式严格校验；失败保留文件和世界；旧版本不误读、不自动迁移 |
| 弱网 | 复用既有九组四进程矩阵，覆盖 RTT 0／100／200 ms、抖动、丢包 0／1／5%及乱序 | 可靠状态收敛、无重复扣款、缓存有界；保存网络扰动参数与四端结果 |
| 容量／性能 | 正常高峰、256 实体／1024 箭矢、完整三夜、胜利后静置；相同前台与硬件条件 | 记录模拟步时、帧时 p95、GC、有效 OS 工作集、序列化大小、带宽和积压；无持续增长，达到 U0 冻结预算 |
...
当前交付只完成本计划和新分支。下一步是 U0 剩余输入归档及 U1 框架适配；本文件中的目标目录、Behaviour、State、配置、协议 6 和存档 v2 均不得被写成已经实现。
