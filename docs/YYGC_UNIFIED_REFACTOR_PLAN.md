...
本计划已进入实施。**U0 已完成输入归档和功能基线；U1 框架适配、分批 Editor／Play、Mono 装配探针、四进程 Sample 及依赖锁定已完成；U2–U6 尚未完成。** 实际证据持续记入[实施记录](YYGC_UNIFIED_IMPLEMENTATION.md)。本计划接续最后一轮架构评估，取代 [C 方案](C_REFACTOR_PLAN.md)作为后续重构方向；C 方案和[场景入口修复](SCENE_DEFINITIONS.md)仍是历史证据。
...
U1 已完成，下一阶段为 U2 工人／采集／建造切片。游戏业务 Behaviour、State、协议 6 和存档 v2 仍未实现；不能因框架接口通过测试就提前标记完整游戏迁移完成。
