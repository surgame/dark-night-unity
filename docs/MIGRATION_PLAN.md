...
2026-09-13 后续重构入口：[YYGC 统一对象架构分阶段执行计划](YYGC_UNIFIED_REFACTOR_PLAN.md)。新分支将运行实体与状态所有权迁入 YYGC，Core 收缩为纯算法／数据合同；可针对框架能力限制和 BUG 升级适配。此次无需 Godot 旧档、当前 Unity v1 存档或旧协议兼容；仍保留冻结玩法、人工资源及新格式完整恢复。**本次仅规划，尚未迁移代码**；下文保留已实施的原移植方案，旧档兼容和 Core 世界的长期保留要求不再适用于新重构目标。
