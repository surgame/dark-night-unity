...
2026-09-12 验收续跑：干净 Mono 构建、活跃状态恢复、正式并发、九组四进程弱网、三夜与容量上限已通过，完整产物与哈希已归档。画面复核发现字体与新增菜单控件需要校准；普通 Player 的工作集、全程分位数与前台观测仍未满足性能签署条件。IL2CPP 授权及第二台 Windows 机器另待外部条件，M5 未完成。见 [Mono 验收记录](MONO_ACCEPTANCE.md)。下文按日期保留历史判断，不作为当前待开发清单。
