...
namespace DarkNights.Core.Logic.Terrain
{
    ...
    public sealed class BackgroundContourBaker
    {
        ...
        public static BackgroundContourBaker Build(BackgroundBakeDescriptor source, Action checkpoint = null, CaveOutlineSettings outline = null)
        {
            ...
            return new BackgroundContourBaker(pixels, w, h, source.LayoutSeed, 43 * 8, checkpoint);
        }

        public BackgroundContourBaker(byte[] occupancy, int width, int height, string seed, int top, Action checkpoint = null)
        {
            ...
            LayoutSeed = Seed(seed + "|DECOR-0922|contour-follow-static-1");
            ...
            for (int gy = 0; gy < gridHeight; gy++)
            {
                ...
                for (int gx = 0; gx < gridWidth; gx++)
                {
                    ...
                    double band = Noise(x / 95.0, y / 18.0, s + 37), edge = Clamp(1 - (d - 1) / (24 * .7));
                    double mid = Clamp(1 - Math.Abs(d - 40 * .75) / (40 * 1.1)), inner = Clamp((d - 4) / (10 * 2.4));
                    ...
                    if (d >= 5.5 && d <= 32 && deepScore > .76 - .48 * .20 - .68 * .08) grids[2][at] = 1;
                    if (d >= 1 && d <= 76 && midScore > .66 - .72 * .18 - .52 * .08) grids[1][at] = 1;
                }
            }
            ...
            for (int gy = 1; gy < gridHeight - 1; gy++) for (int gx = 1; gx < gridWidth - 1; gx++)
            {
                ...
                if (y < top || occupancy[p] != 0 || d < 1 || d > 24 * .7) continue;
                ...
                if (score > .72 - .58 * .16 - .72 * .06) grids[0][at] = 1;
            }
            Cleanup(0);
        }
