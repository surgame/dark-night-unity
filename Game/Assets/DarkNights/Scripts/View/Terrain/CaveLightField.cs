...
namespace DarkNights.View.Terrain
{
    ...
    {
        internal static Color32[] Build(Color32[] cells, int width, int height)
        {
            var warm = new byte[cells.Length]; var cold = new byte[cells.Length];
            for (int i = 0; i < cells.Length; i++) cells[i].b = 0;
            for (int y = 48; y < height - 5; y += 19) for (int x = 14; x < width - 8; x += 23)
            {
            ...
            {
                int i = y * width + x;
                if (cells[i].r >= 4 && cells[i].r <= 6 && cells[i - (y > 0 ? width : 0)].r == 0)
                { cold[i] = 230; cells[i].b = 2; }
            }
        ...
        private static void Spread(byte[] light, Color32[] cells, int width, int height)
        {
