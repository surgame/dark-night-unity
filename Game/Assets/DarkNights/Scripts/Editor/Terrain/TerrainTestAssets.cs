...
{
    /// <summary>仅向新的测试配置目录创建首版 AnyRuleD 资产；图集切片保留参考位序，混合材料使用框架拓扑填充。</summary>
    public static class TerrainTestAssets
    {
        public const string Root = "Assets/DarkNights/Res/Terrain/TestTerrain";
        ...
        private static void SliceAtlas()
        {
            ...
            for (int t = 0; t < 8; t++) for (int v = 0; v < 4; v++) for (int m = 0; m < 16; m++)
                slices[index++] = new SpriteMetaData { name = Keys[t] + "_m" + m + "_v" + v,
                    rect = new Rect(v * 32 + m % 4 * 8, 256 - (t * 32 + m / 4 * 8) - 8, 8, 8),
...
#pragma warning restore CS0618
        ...
        private static string Id(string key)
        {
            ...
            return BitConverter.ToString(hash.ComputeHash(Encoding.UTF8.GetBytes("dark-nights-terrain:" + key)), 0, 16).Replace("-", "").ToLowerInvariant();
        }
