...
namespace DarkNights.Runtime.Objects
{
    ...
    public sealed partial class HeroInventoryBehaviour : PooledBehaviour
    {
        ...
        {
            ActorState state = actor.Edit();
            if (revision != state.SelectionRevision || item != ItemKey(state.SelectedItem) || (item != "jetpack" && item != "pickaxe")) return false;
            if (item == "pickaxe") return HeroMining.TryDeposit(actor, targetId);
