...
namespace DarkNights.Runtime.Objects
{
    ...
    internal static class HeroEquipment
    {
        ...
        internal static void Tick(ActorBehaviour actor, double delta)
        {
            ...
            {
                if (pressed && state.EquipmentCooldown == 0) state.Charging = true;
                ...
                if (released && state.Charging)
                {
                    ...
                    { state.EquipmentCooldown = config.BombCooldown; state.EquipmentAction = 0.25; state.EquipmentActionDuration = 0.25; }
                    Cancel(state);
                }
                return;
            }
