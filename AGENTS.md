...
2026-09-16 当前切片为[主角操控与 YYGC 输入联合执行](docs/HERO_INPUT_EXECUTION.md)：协议 8／存档 v3；YYGC 输入提交 `0c7cec0` 位于隔离 `D:/Developer/YYGC-worktrees/input-actions`，准备脚本已锁定。默认产品入口现固定为主角操控：每个 Ready 玩家由服务端分配一名不同的可用友军，加载、重连及恢复 SharedCamp 后重新分配；遮挡画面的顶部主角工具栏和营地建造、训练、招募、修缮入口暂时隐藏，键盘／鼠标主角快捷键继续生效。旧营地后端、工具栏与 `--dn-camp-mode` 仅为开发回归保留，不作为当前玩家入口。另一个会话的 `D:/Developer/YYGC` master／AnyRule 工作区不能代为切换、清理或合并。此前 173 个不同 Editor／Play、输入 Sample 34 个不同 PlayMode 用例按影响合并通过；本次新增默认分配与工具栏资源用例，受影响 Editor／Play 按组 22/22，累计为 175 个不同用例按影响合并通过。当前 Mono 为 `artifacts/hero-input/player-mono-default-hero-r3`，本机独立 Host＋客户端及两张实际 UI 截图 37/37，默认图已复核不再出现顶部框；第二次 Mono 主矩阵 350 项和道具栏产物相关 155 项只保留原构建身份。不把这些结果写成前台性能、IL2CPP、双机器或 M5 全部完成。
