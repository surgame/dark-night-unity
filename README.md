...
2026-09-21 实验分支 `codex/cave-8x8-art-direction` 的[洞穴 8×8 岩层风格实验](docs/CAVE_8X8_STYLE.md)已增加第二轮：F1 开放洞室尺度、纹理、色板、边缘和灯光参数；默认生成改为 HTML 思路的 4×3 分层受控连接，洞室宽高、层距及通路均收紧；墙缘取消固定深度硬切，灯光改为可调八向柔和传播。工作台及正式远征人物保持 1.5 格高。最终受影响 Editor 19/19 与默认种子实际 Play 截图通过；正式 Player／联机和前台性能未在本批重验，不替代下方当前远征交付结论。
